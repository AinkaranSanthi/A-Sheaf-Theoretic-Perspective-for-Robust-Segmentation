...
class TrainSE3:
    ...
    def Cellular_patches2D(patches, rows, cols):
    ...
    def Cellular_patches3D(patches, rows, cols, dep):
        ...
        for epoch in range(self.epochs):
            with tqdm(range(len(train_dataset))) as pbar:
                for k , imgs in zip(pbar, train_dataset):
                    ...
                    invariantloss += torch.sum((latentt2 - latentadc)**2)
                    ...
                    if epoch > self.topo_epoch:
                       for i in range(lab.shape[0]):
                           for j in range(labt.shape[1]):
                               patchesl = self.divide_tensor_into_patches3D(labt.detach()[i,j], self.patch_size) if self.dim == '3D' else self.divide_tensor_into_patches2D(labt.detach()[i,j], self.patch_size) 
                               patch_lab = self.topo_labels3D(patchesl, patch_dim[0], patch_dim[1], patch_dim[2]) if self.dim == '3D' else self.topo_labels2D(patchesl, patch_dim[0], patch_dim[1])
                               patcheso = self.divide_tensor_into_patches3D(outt[i,j], self.patch_size) if self.dim == '3D' else self.divide_tensor_into_patches2D(outt[i,j], self.patch_size) 
